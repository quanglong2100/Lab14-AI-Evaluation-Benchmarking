...
|------------|---------------|---------------------------|----------------------|
| A | AI/Backend | **Engine & Async Runner**<br>1. Hoàn thiện `engine/runner.py`: tối ưu async, tính cost/token.<br>2. Cài đặt Multi-Judge trong `engine/llm_judge.py` (GPT-4o + Claude 3.5).<br>3. Xử lý logic consensus & agreement rate. | Cần API key của 2 model |
